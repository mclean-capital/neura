...
- Pi enforces Agent Skills spec rules via `loadSkills()`. Neura's `skill-loader.ts` calls pi's loader, then re-parses each returned skill's SKILL.md to extract `allowed-tools` and `metadata.*` (which pi's `Skill` type doesn't expose — confirmed by Spike #4d).

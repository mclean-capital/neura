...
describe('BackupService', () => {
  it('creates a valid backup file', async () => {
    ...
    const service = new BackupService({ store, backupPath });
  ...
  it('restores from backup into a fresh store', async () => {
    ...
    const service2 = new BackupService({ store: store2, backupPath });
  ...
  it('start/stop controls periodic timer', async () => {
    const service = new BackupService({ store, backupPath, intervalMs: 100 });
    service.start();

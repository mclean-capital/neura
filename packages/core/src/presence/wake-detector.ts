...
export class WakeDetector {
  ...
  private readonly preBuffer: Float32Array[] = [];
  ...
  constructor(config: WakeDetectorConfig) {
    ...
    this.ai = new GoogleGenAI({ apiKey: config.googleApiKey });
  }
  ...
  feedAudio(base64Pcm: string): void {
    ...
    if (this.pcmBuffer.length === 0) {
      this.pcmBuffer = float32;
    } else {
      const combined = new Float32Array(this.pcmBuffer.length + float32.length);
      combined.set(this.pcmBuffer);
      combined.set(float32, this.pcmBuffer.length);
  ...
  setAssistantName(name: string): void {
    ...
    log.info('assistant name updated', { assistantName: this.assistantName });
  }
  ...
  private processFrame(frame: Float32Array): void {
    ...
    if (!this.isSpeaking) {
      ...
      if (this.preBuffer.length > PRE_SPEECH_FRAMES) this.preBuffer.shift();
    }
    ...
    if (energy >= SPEECH_THRESHOLD) {
      ...
      if (!this.isSpeaking && this.consecutiveSpeechFrames >= SPEECH_START_FRAMES) {
        this.isSpeaking = true;
        // Prepend pre-speech buffer to capture any clipped onset
        this.speechFrames = [...this.preBuffer];
        this.speechSamples = this.speechFrames.reduce((s, f) => s + f.length, 0);
      ...
      if (this.isSpeaking && this.consecutiveSilenceFrames >= SILENCE_END_FRAMES) {
        this.isSpeaking = false;
        if (this.speechSamples >= MIN_SPEECH_SECONDS * SAMPLE_RATE) {
          void this.checkSpeechSegment();
        } else {
    ...
    if (this.isSpeaking) {
      ...
      // Safety cap
      if (this.speechSamples >= MAX_SPEECH_SECONDS * SAMPLE_RATE) {
        void this.checkSpeechSegment();
        this.speechFrames = [];
        this.speechSamples = 0;
        this.isSpeaking = false;
  ...
  private async checkSpeechSegment(): Promise<void> {
    if (this.closed || this.speechFrames.length === 0) return;
    ...
    try {
      ...
      const isWake = containsWakeWord(transcript, this.assistantName);

      log.info('wake transcript check', { transcript, isWake, assistantName: this.assistantName });
      this.config.onDebug?.({ transcript, isWake });

      if (isWake && !this.closed) {
        log.info('wake word detected in transcript');
        this.config.onWake(replayChunks);
      }
    } catch (err) {

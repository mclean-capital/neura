...
class GeminiService {
  ...
  /**
   * Create a streaming response from Gemini Live API using text prompt
  ...
  /**
   * Create a streaming response from Gemini Live API using audio input
   * @param audioData Audio data buffer and MIME type
   * @returns Readable stream with response chunks
   */
  async streamAudioChat(audioData: AudioData): Promise<Readable> {
    // Create a readable stream to push data to the WebSocket
    const outputStream = new Readable({
      read() {}, // No-op implementation required
    });
    ...
    try {
      ...
      const base64Audio = audioData.data.toString("base64");
